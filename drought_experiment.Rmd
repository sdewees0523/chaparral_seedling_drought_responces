...
ggplot(hourly_stomata_mean, aes(x= time_hour@hour, y = mean, col = treatment, shape = as.factor(date)))+
...
midday_stomata_mean <- midday_stomata %>%
  group_by(species, date, treatment) %>% 
  summarise(mean = ci(stomatal_conducatance)[1],
            lowci = ci(stomatal_conducatance)[2],
            highci = ci(stomatal_conducatance)[3])
