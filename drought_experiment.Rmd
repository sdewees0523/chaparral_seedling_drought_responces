...
soil_moisture_4_14 <- read.csv(here("data", "pulse-data-20220414-154507.csv")) %>% 
  select(Note, Moisture, Timestamp) %>%  
...
soil_moisture_4_18 <- read.csv(here("data", "pulse-data-20220418-134937.csv")) %>% 
  select(Note, Moisture, Timestamp) %>%  
  separate(Note, into = c("species", "id", "treatment"), sep = "-") %>% 
  rename(soil_moisture = Moisture) %>%
  drop_na(id) %>% 
...
gsw_6 <- read.csv(here("data", "gsw light adap", "2022-04-14", "gsw_light_adap_Shane_2022_04_14T21_06_02_083Z_1.csv")) %>% 
  row_to_names(1) %>% 
  filter(!row_number() %in% c(1)) %>% 
  select("species", "id", "gsw", "PhiPS2", "Date") %>% 
  rename(stomatal_conductance = gsw) %>% 
  ...
  mutate(id = as.numeric(id),
         ...
         treatment = case_when(species == "ceol" & id %in% drought_ceol$id ~ "drought",
                               ...
                               species == "hear" & id %in% drought_hear$id ~ "drought", 
                               TRUE ~ "wet"))
gsw_7 <- read.csv(here("data", "gsw light adap", "2022-04-16", "gsw_light_adap_Shane_2022_04_18T22_45_49_611Z_1.csv")) %>% 
  row_to_names(1) %>% 
  filter(!row_number() %in% c(1)) %>% 
  select("species", "id", "gsw", "PhiPS2", "Date") %>% 
  rename(stomatal_conductance = gsw) %>% 
...
gsw_8 <- read.csv(here("data", "gsw light adap", "2022-04-18", "gsw_light_adap_Shane_2022_04_18T22_45_49_663Z_1.csv")) %>% 
  row_to_names(1) %>% 
  filter(!row_number() %in% c(1)) %>% 
  select("species", "id", "gsw", "PhiPS2", "Date") %>% 
  rename(stomatal_conductance = gsw) %>% 
...
gsw_9 <- read.csv(here("data", "gsw light adap", "2022-04-20", "gsw_light_adap_Shane_2022_04_21T18_44_48_845Z_1.csv")) %>% 
  row_to_names(1) %>% 
  filter(!row_number() %in% c(1)) %>% 
  select("species", "id", "gsw", "PhiPS2", "Date") %>% 
  rename(stomatal_conductance = gsw) %>% 
...
gsw_hour_2 <- read.csv(here("data", "gsw light adap", "2022-04-18", "gsw_light_adap_Shane_2022_04_18T22_45_49_663Z_1.csv")) %>% 
  row_to_names(1) %>% 
  filter(!row_number() %in% c(1)) %>% 
  select("species", "id", "gsw", "PhiPS2", "Date", "Time") %>% 
  rename(stomatal_conductance = gsw) %>% 
...
dark_adapted_7 <- read.csv(here("data", "dark adapted", "2022-04-14", "dark_adapted_Shane_Dewees_2022_04_14T21_06_02_120Z_1.csv")) %>% 
  row_to_names(1) %>% 
  filter(!row_number() %in% c(1)) %>% 
  select("species", "plant id", "Fv/Fm", "Date") %>% 
  rename("id" = "plant id") %>% 
...
dark_adapted_8 <- read.csv(here("data", "dark adapted", "2022-04-16", "dark_adapted_Shane_Dewees_2022_04_18T22_45_49_639Z_1.csv")) %>% 
  row_to_names(1) %>% 
  filter(!row_number() %in% c(1)) %>% 
  select("species", "plant id", "Fv/Fm", "Date") %>% 
  rename("id" = "plant id") %>% 
  ...
  mutate(date = ymd(date),
         ...
         water_potential_predawn = NA,
         water_potential_midday = NA)
dark_adapted_9 <- read.csv(here("data", "dark adapted", "2022-04-18", "dark_adapted_Shane_Dewees_2022_04_18T22_45_49_684Z_1.csv")) %>% 
  row_to_names(1) %>% 
  filter(!row_number() %in% c(1)) %>% 
  select("species", "plant id", "Fv/Fm", "Date") %>% 
  rename("id" = "plant id") %>% 
  ...
  mutate(date = ymd(date),
         ...
         water_potential_predawn = NA,
         water_potential_midday = NA)
dark_adapted_10 <- read.csv(here("data", "dark adapted", "2022-04-19", "dark_adapted_Shane_Dewees_2022_04_21T18_44_48_821Z_1.csv")) %>% 
  row_to_names(1) %>% 
  filter(!row_number() %in% c(1)) %>% 
  select("species", "plant id", "Fv/Fm", "Date") %>% 
  rename("id" = "plant id") %>% 
  ...
  mutate(date = ymd(date),
         ...
         water_potential_predawn = NA,
         water_potential_midday = NA)
dark_adapted_11 <- read.csv(here("data", "dark adapted", "2022-04-20", "dark_adapted_Shane_Dewees_2022_04_21T18_44_48_863Z_1.csv")) %>% 
  row_to_names(1) %>% 
  filter(!row_number() %in% c(1)) %>% 
  select("species", "plant id", "Fv/Fm", "Date") %>% 
  rename("id" = "plant id") %>% 
...
```{r, echo=FALSE, message= FALSE, warning=FALSE}
kstem <- read.csv(here("data", "hear_kstem.csv")) %>% 
  rename(species = sp) %>% 
  rename(id = numbr) %>% 
  ...
  mutate(treatment = case_when(id %in% drought_hear$id ~ "drought",
         ...
         kstem = kstem * -1,
         date = mdy("04/18/2022")) %>% 
  select(species, id, kstem, treatment, date)
...
```{r, echo=FALSE, message= FALSE, warning=FALSE}
water_potential <- read.csv(here("data", "flourescence_water_potentials.csv")) %>% 
  filter(is.na(water_potential_predawn) == FALSE) %>% 
  select(!dark_adapted_flourescence) %>% 
  mutate(date = mdy(date)) %>% 
...
```{r, echo=FALSE, message= FALSE, warning=FALSE}
leaf_areas <- read.csv(here("data", "leaf_areas.csv")) %>% 
  rename(species = ï..species) %>% 
  mutate(treatment = case_when(species == "ceol" & id %in% drought_ceol$id ~ "drought",
                               species == "hear" & id %in% drought_hear$id ~ "drought",
                               TRUE ~ "wet"))
stem_areas <- read.csv(here("data", "stem_areas.csv")) %>%
  group_by(species, id) %>% 
